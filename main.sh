...
prompt_for_token_if_missing() {
    ...
    if [[ -z "$token" ]]; then
        ...
        while true; do
            token=$(read_input "Please enter your access token")
            if validate_access_token "$token"; then
                echo "$token" >"$TOKEN_PATH"
                print_message "SUCCESS" "Token saved to $TOKEN_PATH."
                return
            fi
            print_message "ERROR" "Invalid token. Please provide a valid one."
        done
    fi
}
...
ask_for_port() {
    ...
    while true; do
        ...
        if [[ "$operation" != "start" ]] && ! is_service_running "$port"; then
            print_message "ERROR" "No active service found on port $port."
            continue
        fi
...
start_proxy_service() {
    ...
    if confirm_action "n" "$(format_prompt "${EMOJIS[SSL]} Enable SSL?")"; then
        ssl_enabled=":ssl"
        if ! confirm_action "y" "$(format_prompt "${EMOJIS[CERT]} Use internal certificate?")"; then
            ssl_cert_path=$(read_input "Path to SSL certificate")
...
display_menu() {
    ...
    echo -e "${COLORS[TITLE]}║${COLORS[INFO]}[${COLORS[SUCCESS]}04${COLORS[INFO]}] ${COLORS[SUCCESS]}• ${COLORS[ERROR]}VIEW PORT LOGS      ${COLORS[TITLE]}║${COLORS[RESET]}"
