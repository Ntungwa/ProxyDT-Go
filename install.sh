...
download_and_install() {
  BASE_FILENAME="${BINARY_NAME}-${OS_NAME}-${ARCH_NAME}"
  RAW_URL="https://github.com/${REPO}/releases/download/${VERSION}/${BASE_FILENAME}"
  TAR_URL="${RAW_URL}.tar.gz"
  SHA_URL_RAW="${RAW_URL}.sha256"
  SHA_URL_TAR="${TAR_URL}.sha256"

  cd "$TMP_DIR"

  # Try to download the raw binary first
  log_info "Attempting to download binary: $BASE_FILENAME"
  HTTP_STATUS=$(curl -s -w "%{http_code}" -L -o "$BASE_FILENAME" "$RAW_URL")

  if [[ "$HTTP_STATUS" != "200" ]]; then
    log_warn "Raw binary not found. Trying .tar.gz version..."
    HTTP_STATUS=$(curl -s -w "%{http_code}" -L -o "${BASE_FILENAME}.tar.gz" "$TAR_URL")
    if [[ "$HTTP_STATUS" != "200" ]]; then
  ...
  # Try to verify integrity (if SHA file exists)
  if $IS_TAR && curl -s -L -o "${FILENAME}.sha256" "$SHA_URL_TAR"; then
    log_info "Verifying integrity with SHA256..."
    sha256sum -c "${FILENAME}.sha256"
  elif ! $IS_TAR && curl -s -L -o "${FILENAME}.sha256" "$SHA_URL_RAW"; then
    log_info "Verifying integrity with SHA256..."
    sha256sum -c "${FILENAME}.sha256"
  else
  ...
  if $IS_TAR; then
    ...
    tar -xzf "$FILENAME"
    BINARY_EXTRACTED=$(find . -type f -name "$BINARY_NAME*" | head -n 1)
  else
    BINARY_EXTRACTED="$BASE_FILENAME"
  fi

  log_info "Installing binary to $INSTALL_DIR..."
  sudo mv "$BINARY_EXTRACTED" "${INSTALL_DIR}/${BINARY_NAME}"
